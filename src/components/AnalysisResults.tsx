...
const AnalysisResults: React.FC<AnalysisResultsProps> = ({
  ...
  const getOverallRating = () => {
    if (overallScore >= 70) return { label: 'Safe', level: 'safe' as const };
    if (overallScore >= 40) return { label: 'Moderate', level: 'caution' as const };
  ...
  const RiskIcon = ({ level }: { level: 'safe' | 'caution' | 'danger' }) => {
    switch (level) {
      case 'safe':
  ...
  return (
    <div className="w-full animate-slide-up">
          ...
          <div className="space-y-4">
            ...
            <div className="flex items-center justify-between p-4 rounded-xl bg-danger-bg">
              <div className="flex items-center gap-3">
                ...
                <span className="font-medium">Harmfull</span>
              </div>
      ...
      {/* Ingredient List */}
      <div className="bg-card rounded-2xl p-6 shadow-card">
        <h3 className="text-lg font-semibold mb-4">High-Risk Ingredients Alert</h3>
